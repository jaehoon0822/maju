...
// image 생성 router
router.post("/img", isLoggedIn, upload.array("img"), post.createImage);

// 게시글 생성 router
router.post("/", isLoggedIn, upload.none(), post.createPost);
// 게시글 업데이트 router
router.put("/:id", isLoggedIn, upload.none(), post.updatePost);
...
router.get("/get/hashtag", post.getHashtagPosts);
